...
class GoTermsSerializer(serializers.Serializer):
    ...
    go_term_id = serializers.PrimaryKeyRelatedField(
        source="ontology_term", read_only=True
    )
    go_term_name = serializers.ReadOnlyField(source="ontology_term.name")
    qualifier = serializers.ReadOnlyField()
    evidence_code_id = serializers.PrimaryKeyRelatedField(
        source="evidence_code", read_only=True
    )
    evidence_code_name = serializers.ReadOnlyField(source="evidence_code.name")
    assigned_by = serializers.ReadOnlyField()
    quickgo_url = serializers.ReadOnlyField(source="ontology_term.quickgo_url")
